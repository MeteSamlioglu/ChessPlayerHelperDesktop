...
def find_corners(cfg: CN, img: np.ndarray) -> np.ndarray:
    ...
    while iterations < 200 or best_num_inliers < 30:
        ...
        # print(f'{row1} {row2} {col1} {col2} {len(horizontal_lines)} {len(vertical_lines)}')
    ...
    warped = cv2.warpPerspective(img, transformation_matrix, dims)
    ...
    # cv2.imshow("warped_borders",warped_borders)
    # cv2.waitKey(0)

    warped_mask = warped_borders == 1
    
    # for i in range(warped_mask.shape[0]):
    #     for j in range(warped_mask.shape[1]):
    #         print(f'{warped_mask[i][j]}',end = ' ')
    #     print("\n")
...
def _compute_vertical_borders(cfg: CN, warped: np.ndarray, mask: np.ndarray, scale: np.ndarray, xmin: int, xmax: int) -> typing.Tuple[int, int]:
    ...
    myimg = cv2.Sobel(warped, cv2.CV_64F, 1, 0,
                           ksize=cfg.BORDER_REFINEMENT.SOBEL_KERNEL_SIZE)
...
if __name__ == "__main__":
    ...
    cfg = CN.load_yaml_with_base("C:\\Users\\Monster\\Desktop\\Graduation Project1\\Project\\Desktop Application\\configuration\\corner_detection.yaml")
    
    filename = URI("D:\\chesscog\\example\\myboard.jpg")
    ...
    fig = plt.figure()
    fig.canvas.setWindowTitle("Corner detection output")
